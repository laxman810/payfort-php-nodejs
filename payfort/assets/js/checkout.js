function getPaymentPage(paymentMethod) {
    ...
    $.ajax({
        ...
        type: 'post',
        dataType: 'json',
        data: {paymentMethod: paymentMethod, userId: $('#UserId').val() ,userType : $('#UserType').val()},
        success: function (response) {
            if (response.form) {
        	    ...
        	    var expdatearr = $('#payfort_fort_mp2_expiry').val().split('/');
                    var expDate = expdatearr[1]+''+expdatearr[0];//$('#payfort_fort_mp2_expiry').val();//$('#payfort_fort_mp2_expiry_year').val() + '' + $('#payfort_fort_mp2_expiry_month').val();
                    var mp2_params = {};
